...
def send_whatsapp_alert():
    account_sid = '#votre account_side(chercher le dans twilio)'  # Remplacez par votre account SID Twilio
    auth_token = '#votre tocken(chercher le danstwilio)'  # Remplacez par votre token d'authentification Twilio
    client = Client(account_sid, auth_token)
    message = client.messages.create(
        from_='whatsapp:+14155238886',
        body='Il y a une alerte importante sur votre Capteur : la température dépasse le seuil.',
        to='whatsapp:+212628305231'  # Remplacez par votre numéro WhatsApp
    )
    return message
    ...
    if request.method == "GET":
        ...
        if serial.is_valid():
            ...
            if new_data.temp > 10:  # Remplacez 10 par votre seuil
                # Alerte Telegram
                telegram_token = 'votre_telegra_bot_tocken'  # Remplacez par votre token Telegram
                chat_id = 'your_chat_id'  # Remplacez par votre chat ID
                ...
                recipient_list = ['elboudalihamza13@gmail.com']  # Remplacez par votre e-mail de destination
